...
class BaseTrainer:
    ...
    def save_model(self):
        ...
        def _finite_state_dict(module) -> bool:
            return all(
                torch.isfinite(v).all() for v in unwrap_model(module).state_dict().values() if isinstance(v, torch.Tensor)
            )

        ema_src = unwrap_model(self.ema.ema)
        if not _finite_state_dict(ema_src):
            LOGGER.warning(
                f"EMA contains NaN/Inf at epoch {self.epoch}, falling back to current model weights for checkpoint"
            )
            ema_src = unwrap_model(self.model)
            if not _finite_state_dict(ema_src):
                LOGGER.warning(f"Skipping checkpoint save at epoch {self.epoch}: model weights also contain NaN/Inf")
                return False

        ema = deepcopy(ema_src).half()
