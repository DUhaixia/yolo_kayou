...
class SegmentationValidator(DetectionValidator):
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        ...
        min_stride = self._resolve_min_stride(model)
    ...
    @staticmethod
    def _resolve_min_stride(model: torch.nn.Module) -> int | None:
        """Minimum detect stride (P2=4, P3-only=8). AutoBackend.stride is max stride int — use inner model."""
    ...
    def _min_stride(self) -> int:
        if self.stride is not None:
            s = self.stride
            if isinstance(s, torch.Tensor):
                return int(s.min().item())
            if isinstance(s, (list, tuple)):
                return int(min(s))
            return int(s)
        raise RuntimeError("SegmentationValidator.stride is unset; init_metrics(model) must run before validation.")

    def _mask_hw(self, letterbox_hw: tuple[int, int]) -> tuple[int, int]:
        """Mask grid size for proto output (P2 stride=4 -> imgsz/2, P3 stride=8 -> imgsz/4)."""
        s = self._min_stride()
        return letterbox_hw[0] * 2 // s, letterbox_hw[1] * 2 // s
    ...
    def postprocess(self, preds: list[torch.Tensor]) -> list[dict[str, torch.Tensor]]:
        ...
        mh, mw = proto.shape[2:]
        s = self._min_stride()
        imgsz = [mh * s // 2, mw * s // 2]  # letterbox size (P2/P3-safe; was 4*proto, wrong for P2)
        for i, pred in enumerate(preds):
            ...
            pred["masks"] = (
                ...
                if coefficient.shape[0]
                    ...
                    (0, *(imgsz if self.process is ops.process_mask_native else (mh, mw))),
                    dtype=torch.uint8,
    ...
    def _prepare_batch(self, si: int, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if nl:
            if self.process is ops.process_mask_native:
                mask_size = [int(s) for s in prepared_batch["imgsz"]]
            else:
                mask_size = list(self._mask_hw(tuple(int(s) for s in prepared_batch["imgsz"])))
