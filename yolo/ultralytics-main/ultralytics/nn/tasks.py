...
from ultralytics.nn.modules.ses_net import collect_ses_aux_loss, set_ses_targets
...
def _shift_pretrained_keys_for_ses_input_cache(csd: dict, state_dict: dict) -> dict:
    """Remap model.{i}.* -> model.{i+1}.* when layer 0 is SESInputCache (pass-through, no weights).

    Without this, intersect_dicts matches wrong layers: ckpt model.1 loads into new model.1 instead of the
    first Conv at new model.1 that should receive ckpt model.0.
    """
    if not state_dict or not any(k.startswith("model.0.") for k in state_dict):
        return csd
    first = state_dict.get("model.0.conv.weight")
    if first is not None:
        return csd  # standard YOLO: layer 0 is Conv
    shifted = {}
    for k, v in csd.items():
        ...
        try:
            ...
            continue
        nk = f"model.{idx + 1}.{parts[2]}"
        if nk in state_dict and state_dict[nk].shape == v.shape:
            shifted[nk] = v
    return shifted or csd
...
class BaseModel(torch.nn.Module):
    ...
    def load(self, weights, verbose=True):
        ...
        csd = _shift_pretrained_keys_for_ses_input_cache(csd, self.state_dict())
    ...
    def loss(self, batch, preds=None):
        ...
        if preds is None:
            set_ses_targets(self.model, batch)
            preds = self.forward(batch["img"])
        loss, loss_items = self.criterion(preds, batch)
        ses = collect_ses_aux_loss(self.model)
        if ses is not None and torch.isfinite(ses).all():
            gain = float(getattr(self.args, "ses_gain", 0.1))
            loss = loss + ses * gain * batch["img"].shape[0]
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is HGBlock:
                ...
                n = 1
        elif m in {ResNetLayer, DCFAStage}:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m is SESInputCache:
            c2 = ch[f]
            args = []
        elif m is ACFEEBlock:
            c2 = ch[f]
            args = [ch[f], *args]
        elif m is SESFusion:
            if isinstance(f, list):
                ...
                c_feat, img_ch = ch[f], ch
            c2 = c_feat
            args = [c_feat, *args, img_ch]
        elif m is SmallDefectEnhance:
            if isinstance(f, list):
                c_feat = ch[f[1]]
                img_ch = ch[f[0]]
            else:
            ...
            if isinstance(f, list):
                ...
                c_feat, img_ch = ch[f], 3
            c2 = c_feat
            args = [c_feat, *args, img_ch]
        elif m in {ANUP, ANUPNorm}:
            c2 = ch[f]
        elif m is DPSC:
            c2 = ch[f]
            args = [ch[f]]
